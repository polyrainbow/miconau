...
pub fn listen(
    ...
    let in_port = match in_ports.len() {
        ...
        _ => {
            match input_port_index {
                Some(input_port_index) => {
                    in_ports.get(input_port_index as usize)
    ...
    let conn_in = midi_in.connect(in_port, "midir-read-input", move |_stamp, message, _ | {
        if message[0] == 144 && message[2] > 0 { // if it's a noteOn message with velocity higher than 0
            let note = message[1];
            tx.send(note).unwrap();
        }
    }, ())?;
